...
export function OrderDetailsHistory({ history }: Props) {
  const renderSummary = () => (
    ...
    >
      <Stack spacing={0.5}>
        <Box sx={{ color: 'text.disabled' }}>주문 시간</Box>
        {fDateTime(history?.orderTime)}
      </Stack>

      <Stack spacing={0.5}>
        <Box sx={{ color: 'text.disabled' }}>결제 시간</Box>
        {fDateTime(history?.paymentTime)}
      </Stack>
      <Stack spacing={0.5}>
        <Box sx={{ color: 'text.disabled' }}>배송 시작 시간</Box>
        {fDateTime(history?.deliveryTime)}
      </Stack>

      <Stack spacing={0.5}>
        <Box sx={{ color: 'text.disabled' }}>배송 완료 시간</Box>
      ...
      {history?.timeline.map((item, index) => {
        ...
        return (
          <TimelineItem key={item.title}>
            <TimelineSeparator>
              <TimelineDot color={(lastTime && 'primary') || 'grey'} />
